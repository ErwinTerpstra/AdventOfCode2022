...
class Point:
	...
	def __hash__(self) -> int:
		return ((self.y & 0xFF) << 8) | (self.x & 0xFF)
...
class State:
	...
	def hash(cls, state) -> int:
		return (hash(state.position) << 12) | ((state.tick & 0x3FF) << 2) | (state.phase & 0x03)
...
targetPosition = Point(width - 1, height)

initialState = State(0, 0, startPosition)
...
while len(openList) > 0:
	...
	for offset in DIRECTION_OFFSETS:
		...
		currentTarget = startPosition if state.phase == 1 else targetPosition

		if newPosition == currentTarget:
			newPhase += 1

			# Check if we did our required three trips
			if newPhase == 3:
		...
		# Create new state
		newState = State(newTick, newPhase, newPosition)
